...
@protocol GC_MKOfflineMapDelegate;

#if MAP_TYPE == MAP_GOOGLE
...
#endif
///离线地图服务
@interface GC_MKOfflineMap : NSObject

@property (nonatomic, retain) id<GC_MKOfflineMapDelegate> delegate;
...
/**
 *返回热门城市列表
 *@return 热门城市列表,用户需要显示释放该数组，数组元素为GC_MKOLSearchRecord
 */
- (NSArray*)getHotCityList;

/**
 *返回所有支持离线地图的城市列表
 *@return 支持离线地图的城市列表,用户需要显示释放该数组，数组元素为GC_MKOLSearchRecord
 */
 ...
 *根据城市名搜索该城市离线地图记录
 *@param cityName 城市名
 *@return 该城市离线地图记录,用户需要显示释放该数组，数组元素为GC_MKOLSearchRecord
 */
- (NSArray*)searchCity:(NSString*)cityName;

/**
 *返回各城市离线地图更新信息
 *@return 各城市离线地图更新信息,用户需要显示释放该数组，数组元素为GC_MKOLUpdateElement
 */
...
///离线地图delegate，用于获取通知
@protocol GC_MKOfflineMapDelegate<NSObject>
/**
 *返回通知结果
