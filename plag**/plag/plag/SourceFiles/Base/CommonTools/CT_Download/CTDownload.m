...
- (void)start
{
    ...
    //目标地址与缓存地址
        ...
        NSString *urlString =[CTUtility getCachePath];
        urlString=[urlString stringByAppendingPathComponent:APP_CACHE_FOLDER];
        
        NSFileManager *fm=[NSFileManager defaultManager];
        if (![fm fileExistsAtPath:urlString])
        {
            NSError* error = nil;
            [fm createDirectoryAtPath:urlString withIntermediateDirectories:YES attributes:nil error:&error];
        }
        
        urlString=[urlString stringByAppendingPathComponent:fileName];

    
    destinationPath= [urlString retain];//[[filePath stringByAppendingPathComponent:fileName] retain];
    
	temporaryPath=[[destinationPath stringByAppendingFormat:kTHDownLoadTask_TempSuffix] retain];
...
//                [delegate downloadFinished:self];
    ...
    self.fileHandle = [NSFileHandle fileHandleForWritingAtPath:temporaryPath];
