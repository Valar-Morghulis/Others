...
+ (NSString*)UDID
{
    ...
    NSString *udid = [CTUDIDTools getUDIDFromKeyChain];
    
    
    if (udid == nil) {
        
        NSUserDefaults *userdefaults=[NSUserDefaults standardUserDefaults];
        NSString *oldUUID=[userdefaults objectForKey:@"uuid"];
        
        if (oldUUID != nil) {
            udid = [[[NSString alloc] initWithString:oldUUID] autorelease];
        }
        else
        {
            if (version >= 6.0)
            ...
            else
            {
                ...
                udid = [[[NSString alloc] initWithString:(NSString *)string] autorelease];
        ...
        [CTUDIDTools settUDIDToKeyChain:udid];
...
+ (NSString*)_UDID_iOS6
{
    return [CTUDIDTools getMacAddress];
}
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([CTUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [CTUDIDTools updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
