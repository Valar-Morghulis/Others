...
@interface UIView (CECollectionView)
...
@implementation UIView (CECollectionView)
...
@interface CECollectionViewTapGestureRecognizer : UITapGestureRecognizer
@end

@implementation CECollectionViewTapGestureRecognizer
@end


@interface CECollectionView () <UIGestureRecognizerDelegate>
 ...
 */
- (void)enqueueReusableView:(CECollectionViewCell *)view;
...
@implementation CECollectionView
...
- (void)relayoutViews {
    ...
    [self.visibleViews enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        CECollectionViewCell *view = (CECollectionViewCell *)obj;
    ...
    CGFloat top = _marginY;
    CGFloat totalHeight = 0;
    
    // Add headerView if it exists
    if (self.headerView)
    {
        self.headerView.top = _marginY;
    ...
    if (numViews > 0)
    {
        ...
        self.colWidth = floorf((self.width - _marginX * (self.numCols + 1)) / self.numCols);
        for (NSInteger i = 0; i < numViews; i++) {
            ...
            CGFloat left = _marginX + (col * _marginX) + (col * self.colWidth);
            CGFloat top = [[colOffsets objectAtIndex:col] floatValue];
            CGFloat colHeight = [self.collectionViewDataSource heightForViewAtIndex:self index:i];
            if (colHeight == 0) {
            ...
            CGRect tagFrame = viewRect;
            tagFrame.origin.x = (int) tagFrame.origin.x;
            tagFrame.origin.y = (int) tagFrame.origin.y;
            tagFrame.size.width = (int) tagFrame.size.width;
            tagFrame.size.height = (int) tagFrame.size.height;
            
            [self.indexToRectMap setObject:NSStringFromCGRect(tagFrame) forKey:key];
            
            // Update the last height offset for this column
            CGFloat test = top + colHeight + _marginY;
    ...
    } else
    {
        ...
        if (self.emptyView) {
            self.emptyView.frame = CGRectMake(_marginX, top, self.width - _marginX * 2, self.height - top - _marginY);
...
- (void)removeAndAddCellsIfNecessary {
    ...
    for (NSInteger i = topIndex; i < bottomIndex; i++) {
        ...
        if (![self.visibleViews objectForKey:key] && CGRectIntersectsRect(visibleRect, rect)) {
            ...
            CECollectionViewCell *newView = [self.collectionViewDataSource collectionView:self viewAtIndex:i];
            newView.frame = CGRectFromString([self.indexToRectMap objectForKey:key]);
            [self addSubview:newView];
            
            // Setup gesture recognizer
            if ([newView.gestureRecognizers count] == 0) {
                CECollectionViewTapGestureRecognizer *gr = [[[CECollectionViewTapGestureRecognizer alloc] initWithTarget:self action:@selector(didSelectView:)] autorelease];
                gr.delegate = self;
...
- (CECollectionViewCell *)dequeueReusableView:(NSString *)reuseIdentifier
{
    CECollectionViewCell *res = 0;
    for(CECollectionViewCell *view in self.reuseableViews)
    {
        if(view && [view._reuseIdentifier isEqualToString:reuseIdentifier])
        {
            [view retain];
            [self.reuseableViews removeObject:view];
            res = [view autorelease];
            break;
        }
    }
    return res;
}
- (CECollectionViewCell *)dequeueReusableView {
    CECollectionViewCell *view = [self.reuseableViews anyObject];
    if (view) {
...
- (void)didSelectView:(UITapGestureRecognizer *)gestureRecognizer {
    CGRect tagFrame = gestureRecognizer.view.frame;
    tagFrame.origin.x = (int) tagFrame.origin.x;
    tagFrame.origin.y = (int) tagFrame.origin.y;
    tagFrame.size.width = (int) tagFrame.size.width;
    tagFrame.size.height = (int) tagFrame.size.height;
    NSString *rectString = NSStringFromCGRect(tagFrame);
    ...
    if ([gestureRecognizer.view isMemberOfClass:[[self.visibleViews objectForKey:key] class]]) {
        if (self.collectionViewDelegate && [self.collectionViewDelegate respondsToSelector:@selector(collectionView:didSelectView:atIndex:)]) {
            ...
            [self.collectionViewDelegate collectionView:self didSelectView:(CECollectionViewCell *)gestureRecognizer.view atIndex:matchingIndex];
        }
    }
}

- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if (![gestureRecognizer isMemberOfClass:[CECollectionViewTapGestureRecognizer class]]) return YES;
